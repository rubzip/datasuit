...
class SortOperation(Operation):
    ...
    def __validate_ascending(self):
        if len(self.ascending) != len(self.columns):
            raise ValueError(
                "Ascending must be a list of booleans with the same length as columns"
            )
    ...
    def apply(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        return df.sort_values(
            by=self.columns, ascending=self.ascending, na_position=self.na_position
        )
    ...
    def to_code(self) -> List[str]:
        if not self.columns:
            return []
        return [
            f"df = df.sort_values(by={self.columns}, ascending={self.ascending}, na_position='{self.na_position}')"
        ]
